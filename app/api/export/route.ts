...
export async function POST(req: NextRequest) {
  try {
    ...
    // Tailored summary: start from original, apply latest accepted summary_rewrite
    let tailoredSummary =
    ...
    // Tailored bullets: flatten experience bullets; for each, use accepted bullet_rewrite if any
    ...
    for (const exp of parsed.experience ?? []) {
      for (const b of exp.bullets ?? []) {
        const key = b.trim();
        allBullets.push(
          bulletRewrites.has(key) ? bulletRewrites.get(key)! : b
        );
      }
    }

    // Keywords: JD skills + domain_keywords + accepted keyword_addition / skills_adjustment (dedupe, sort)
    const jdSource = [
    ...
    const fromSuggestions = accepted
      .filter(
        (s) =>
          s.type === "keyword_addition" || s.type === "skills_adjustment"
      )
      .flatMap((s) =>
        s.suggestedText
          .split(/[\n,]+/)
